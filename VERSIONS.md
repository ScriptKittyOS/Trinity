...
| Component | Pin | Note |
|---|---|---|
| `erlang` | **28.5.0.5** | Newest OTP present on all four Burrito targets. Measured at slice 000; see ADR-0005's second correction. |
| `elixir` | **1.20.4-otp-28** | Built-in type checker is part of the gate. |

### Dependencies added at slice 000

| Package | Pin | Note |
|---|---|---|
| `boundary` | **~> 0.10** | Compile-time boundary enforcement. Enforces ONLY under --warnings-as-errors; measured at slice 000. |
| `credo` | **~> 1.7** | --strict in the gate; hosts the eval-family check. |
| `mox` | **~> 1.2** | Mocks for every behaviour. |
| `mix_audit` | **~> 2.1** | Known vulnerabilities in the lock. |
| `sobelow` | **~> 0.15** | Phoenix security lint, blocking with a committed skip list. |
| `ex_doc` | **~> 0.38** | Docs. |
| `nimble_options` | **~> 1.1** | Option validation for behaviours. |
<!-- versions:end -->
