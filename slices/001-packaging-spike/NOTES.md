...
## Item 1 — the owner's results, and what measuring the 5-second bound actually found

Dated 2026-09-07. Owner's report from the X11 desktop, verbatim in the slice issue.

**A. AC4 is proven.** `mix ex_tauri.dev` opened a window titled **Trinity** showing the Phoenix
scaffold. The first native window in this project, on any OS.

**B. AC6, first paint: 2–3 seconds** from Enter to the page appearing. Timed by hand, which is
the only way a first paint can be timed, and stated to the precision it was taken at.

**C. The screenshot.** `xdotool` is absent, so the `-window` form in my step C could not resolve
a window id and `import` wrote a **185-byte stub** — a file that exists, is named like evidence,
and contains nothing. Since overwritten by `import -window root`:

```
slices/001-packaging-spike/proof/ac4-linux-window.png  2814779 bytes, PNG 7680x2160
sha256 e52785ce61ea9f69615ffa7602f7fd172b1fa5a310d8b69b1b11f7fc09bfabe7
```

**The stub is worth recording because of what it nearly was.** I handed over a command whose
failure mode is a plausible-looking artefact rather than an error, and I had checked that
`import` was present without checking that `xdotool` was. A 185-byte PNG committed under
`proof/` would have satisfied every check this project has: the file exists, the path is right,
`trinity.reuse` covers it, nothing reads its contents. **Size and digest are in PROOF.md for
exactly that reason** — they are the only things that distinguish the artefact from the stub.

**D. AC8 passes through the window**, and **my stated pass condition was wrong.** I told the
owner `exit=1` from `grep` was the pass. My grep pattern matches `beam.smp`, and the owner's own
session runs one, so the pass condition could never have been met. The owner's reading is the
correct one and is the one recorded:

```
# before closing, window open
 966625  966348 beam.smp            125     <- mix ex_tauri.dev
 966653  966625 erl_child_setup     125
 966776  966653 cargo-tauri         125
 966847  966776 trinity             125     <- the Rust window
 967584  966847 beam.smp            123     <- the sidecar
 967635  967584 erl_child_setup     123
 2487028 2485809 beam.smp          14543    <- this session's own BEAM
 2487056 2487028 erl_child_setup   14543
...
Shell, sidecar and dev task all gone; the two survivors are the session's own, same pids,
present before launch. **The recorded check excludes them by pid** rather than relying on an
exit code that cannot be reached:

```
ps -eo pid,ppid,comm,etimes | grep -E 'desktop_linux|desktop-x86_64|beam.smp|cargo-tauri|trinity' \
  | grep -v grep | grep -vE '^ *(2487028|2487056) '
```

### The 5-second bound, measured. It is not 5 seconds and it is not the heartbeat.

Six runs on the dev path: launch `mix ex_tauri.dev`, wait for the window and its sidecar, wait
3 s so the heartbeat has connected at least once, kill the `trinity` process, poll until the
sidecar's `beam.smp` is gone.

```
$ SIG=-TERM heartbeat.sh   (three runs)
19 sidecar=983653 trinity=982890 kill_exit=0
15 sidecar=986660 trinity=985757 kill_exit=0
14 sidecar=989056 trinity=988161 kill_exit=0

$ SIG=-KILL heartbeat.sh   (three runs)
16 sidecar=991816 trinity=991114 kill_exit=0
15 sidecar=993309 trinity=992603 kill_exit=0
14 sidecar=994794 trinity=993941 kill_exit=0
```

**14–19 ms against a 5 000 ms bound.** AC8 passes on the dev path with three hundred times the
margin.

**And the heartbeat is not what did it.** `ExTauri.ShutdownManager`'s timer is 1500 ms; every
figure above is two orders of magnitude under it, so the timer never fired. `SIGKILL` gives the
same figure as `SIGTERM`, which rules out graceful cleanup by the Rust process — under `-KILL`
it runs no cleanup at all. Something faster and OS-level ends the sidecar: the stdio pipe
closing when its parent dies, which is **the first of the two candidates finding F1 already
names**, happening by accident of how Tauri spawns a child rather than by design.

### The dev sidecar is not the production sidecar, and that changes the answer

```
$ cat burrito_out/desktop-x86_64-unknown-linux-gnu
#!/bin/sh
# Dev sidecar generated by ExTauri...
cd "/home/aylac/Projects/Trinity" || exit 1
exec mix phx.server
```

**In development the sidecar is a shell script running `mix phx.server`. In production it is the
Burrito binary** — a wrapper process that launches a BEAM as its child. Those are different
process shapes, and finding F1 is precisely about the second one.

Measured on the production shape, three runs — kill the wrapper, which is exactly what Tauri
does to its sidecar on window close:

```
$ prodkill.sh   (three runs)
...
**Orphaned every time, past 7.6 s, still answering HTTP 200.** Well outside the 5 s bound.

**Why the heartbeat does not save it, and this is the mechanism worth carrying to slice 100.**
`ExTauri.ShutdownManager` does not enforce its timeout until the frontend has connected at least
once — deliberately, so a slow boot cannot shut the app down before its window attaches. In this
measurement no Tauri frontend ever connected, so the timeout is never armed and the orphan sits
there indefinitely.

**What that does and does not establish.** It establishes that a production-shaped sidecar whose
window never attached orphans and keeps serving — F1 reproduced on the current build, with the
heartbeat compiled in. It does **not** establish what happens to a production sidecar whose
window attached and then closed: there the heartbeat would be armed, and I could not form that
process tree. Two attempts failed for mechanical reasons — Tauri's dev mode waits on `devUrl`
before launching, and my second attempt matched WebKit's process instead of the sidecar. **That
measurement is unmade, and it is the one that decides whether F1 is closed.**
...
### The commit-msg path check, and what it caught

`.githooks/commit-msg` gains a **warning-only** check: every repository path the message names
should appear in the staged diff. Two tiers, because one is not enough:

* **WARNING** — the path exists nowhere in the tree. Catches an invented or mistyped path.
* **NOTE** — the path exists but this commit does not touch it. Noisy by design, and it is the
  tier that matters.

**It never blocks.** A message legitimately names paths it does not change — the file a fix
refers to, a command being quoted — so a blocking version would be wrong far more often than
right. Its job is to put the claim beside the diff at the moment of writing.

**What it caught the first time it fired: `a6085b3`, replayed.** That commit's message said
`Cargo.lock` had been added to `REUSE.toml` when the edit had failed on a stale anchor. Tier one
would have missed it — `REUSE.toml` exists, so "does this path exist?" says nothing. Tier two
catches it:

```
$ sh .githooks/commit-msg <a6085b3's message>     # staged diff without REUSE.toml
