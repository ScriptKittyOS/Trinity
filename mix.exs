  ...
  defp deps do
    exla_deps() ++
      [
        ...
        # Slice 002: the CycloneDX bill of materials the gate generates. Dev and test only:
        # it produces a record about the build and ships in no artifact.
  ...
  defp aliases do
    [
      ...
      gate: [
        ...
        # Slice 002: the bill of materials is generated by the gate, so a bill that cannot
        # be produced fails the build rather than being quietly missing from a release.
        "trinity.sbom",
        # Slice 120: the third-party licence list is derived from the bill above, so it runs after
        # it and cannot disagree with it. Pure Elixir, so unlike `reuse lint` it runs on every leg
        # including the FIPS container.
        "trinity.third_party_licenses --check",
        "credo --strict",
        "sobelow --exit --skip",
